...
@st.cache_data
def load_data():
    # Certifique-se de que o arquivo CSV esteja na mesma pasta do script
    df = pd.read_csv('Case_Midia_Produto.csv')
    return df

df = load_data()

# Métricas consolidadas para os gráficos
...
with col_text1:
    ...
    **Principais Insights:**
    ...
    - A **Mídia Programática** apresenta o menor ROAS (0.25), indicando que o tráfego de baixo custo não está convertendo em vendas diretas.
...
with col_graf2:
    ...
    st.warning("Ponto de Atenção: Influenciadores em Perfumaria operam abaixo do break-even (ROAS 0.84).")
...
# Criando 3 caixas de destaque para a estratégia
rec_col1, rec_col2, rec_col3 = st.columns(3)

with rec_col1:
    st.success("🚀 **Prioridade 1: Google Search**")
    st.write("Alocação sugerida: **R$ 35.000**")
    st.write("Foco: Termos de fundo de funil para Perfumaria (ROAS 9.8x).")

with rec_col2:
    st.warning("📸 **Prioridade 2: Influenciadores**")
    st.write("Alocação sugerida: **R$ 10.000**")
    st.write("Foco: Escalar Maquiagem e manter o branding em Perfumaria.")
